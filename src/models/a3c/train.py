...
def train(rank, params, shared_model, optimizer):
    ...
    model = A3C(1, params.num_actions).cuda()
    ...
    while True:
        ...
        if done: # if it is the first iteration of the while loop or if the game was just done, then:
            cx = Variable(torch.zeros(1, 256)).cuda() # the cell states of the LSTM are reinitialized to zero
            hx = Variable(torch.zeros(1, 256)).cuda() # the hidden states of the LSTM are reinitialized to zero
        else: # else:
            cx = Variable(cx.data).cuda() # we keep the old cell states, making sure they are in a torch variable
            hx = Variable(hx.data).cuda() # we keep the old hidden states, making sure they are in a torch variable
        ...
        for step in range(params.num_steps): # going through the num_steps exploration steps
            ...
            if done:
                ...
                trainer.new_episode() # we restart the environment
        ...
        if not done: # if we are not done:
            value, _, _ = model((Variable(state.unsqueeze(0)).cuda(), (hx, cx))) # we initialize the cumulative reward with the value of the last shared state
